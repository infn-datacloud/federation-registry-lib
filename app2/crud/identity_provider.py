...
def create_identity_provider(tx: ManagedTransaction, *args, **kwargs) -> IdentityProvider:
    s = ", ".join(
        [f"p.{k}=${k}" for k in IdentityProvider.__fields__.keys() if k != "id"]
    )
...
def get_identity_providers(tx: ManagedTransaction, *args, **kwargs) -> List[IdentityProvider]:
    cypher = """
...
def get_identity_provider(tx: ManagedTransaction, id: UUID) -> Optional[IdentityProvider]:
    cypher = """
...
def get_identity_provider_by_name(tx: ManagedTransaction, name: str) -> IdentityProvider:
    cypher = """
