...
class Region(StructuredNode):
    ...
    location = RelationshipTo(
        "fedreg.v1.location.models.Location", "LOCATED_AT", cardinality=ZeroOrOne
    )
    provider = RelationshipFrom(
        "fedreg.v1.provider.models.Provider", "DIVIDED_INTO", cardinality=One
    )
    services = RelationshipTo(
        "fedreg.v1.service.models.Service", "SUPPLY", cardinality=ZeroOrMore
    )
