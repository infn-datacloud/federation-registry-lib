...
from models.cmdb import ProviderRead, ProviderWrite
...
def add_or_patch_provider(
    *, cmdb_urls: URLs, provider: ProviderWrite, token: str
) -> None:
    ...
    if resp.status_code == status.HTTP_200_OK:
        ...
        if len(provider_list) == 0:
            ...
            db_provider = crud_provider.create(
                new_data=provider, url=cmdb_urls.providers, header=write_header
            )
            ...
            url = os.path.join(cmdb_urls.providers, str(db_provider.uid))
            ...
            for project in provider.projects:
                ...
                for quota in [project.compute_quota, project.block_storage_quota]:
                    ...
                    if db_quota is not None:
                        ...
                        url = os.path.join(cmdb_urls.quotas, str(db_quota.uid))
            ...
            for identity_provider in provider.identity_providers:
                ...
                if db_identity_provider is None:
                    ...
                    url = os.path.join(
                        cmdb_urls.identity_providers, str(db_identity_provider.uid)
                    )
            ...
            if provider.location is not None:
                ...
                if db_location is None:
                    ...
                    url = os.path.join(cmdb_urls.locations, str(db_location.uid))
