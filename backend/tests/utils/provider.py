...
def validate_public_attrs(*, obj_in: ProviderBase, db_item: Provider) -> None:
    assert obj_in
    assert db_item
...
def validate_attrs(*, obj_in: ProviderBase, db_item: Provider) -> None:
    validate_public_attrs(obj_in=obj_in, db_item=db_item)


def validate_rels(
    *,
    obj_out: Union[ProviderReadExtended, ProviderReadExtendedPublic],
    db_item: Provider,
) -> None:
    ...
    for db_idp, idp_out in zip(
        sorted(db_item.identity_providers, key=lambda x: x.uid),
        sorted(obj_out.identity_providers, key=lambda x: x.uid),
    ):
...
def validate_create_provider_attrs(
    *, obj_in: ProviderCreateExtended, db_item: Provider
    ...
    for db_proj, proj_in in zip(
        sorted(db_item.projects, key=lambda x: x.uuid),
        sorted(obj_in.projects, key=lambda x: x.uuid),
    ):
    ...
    for db_idp, idp_in in zip(
        sorted(db_item.identity_providers, key=lambda x: x.endpoint),
        sorted(obj_in.identity_providers, key=lambda x: x.endpoint),
    ):
        validate_create_identity_provider_attrs(obj_in=idp_in, db_item=db_idp)
    assert len(db_item.regions) == len(obj_in.regions)
    for db_reg, reg_in in zip(
        sorted(db_item.regions, key=lambda x: x.name),
        sorted(obj_in.regions, key=lambda x: x.name),
    ):
