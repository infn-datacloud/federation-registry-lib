...
def test_read_identity_providers_with_target_params(
    db_idp_with_single_user_group: IdentityProvider, api_client_read_only: TestClient
) -> None:
...
def test_read_sorted_identity_providers(
    ...
    response = api_client_read_only.get(
        f"{settings.API_V1_STR}/identity_providers/", params={"sort": "uid_desc"}
    )
    assert response.status_code == status.HTTP_200_OK
    content = response.json()
    assert len(content) == 2
...
def test_read_not_existing_identity_provider(api_client_read_only: TestClient) -> None:
...
def test_patch_identity_provider(
    db_idp_with_single_user_group: IdentityProvider, api_client_read_write: TestClient
) -> None:
