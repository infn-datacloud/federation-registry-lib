...
def test_read_not_existing_user_group(
    request: pytest.FixtureRequest, client: TestClient, public: bool
) -> None:
...
def test_read_user_groups_with_target_params(
    ...
    """Execute GET operations to read all User Groups matching specific attributes.
    ...
    for k in UserGroupBase.__fields__.keys():
        a.read_multi(
            client=request.getfixturevalue(client),
            db_items=[db_user_group2],
            params={k: db_user_group2.__getattribute__(k)},
            public=public,
        )
...
@pytest.mark.parametrize("client, public", CLIENTS_FAILING_AUTHN + CLIENTS_READ_ONLY)
