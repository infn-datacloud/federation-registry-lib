...
def test_read_network_services_with_target_params(
    db_network_serv: NetworkService, api_client_read_only: TestClient
) -> None:
...
def test_read_sorted_network_services(
    ...
    response = api_client_read_only.get(
        f"{settings.API_V1_STR}/network_services/", params={"sort": "uid_desc"}
    )
    assert response.status_code == status.HTTP_200_OK
    content = response.json()
    assert len(content) == 2
...
def test_read_not_existing_network_service(api_client_read_only: TestClient) -> None:
...
def test_patch_network_service(
    db_network_serv: NetworkService, api_client_read_write: TestClient
) -> None:
...
def test_patch_not_existing_network_service(api_client_read_write: TestClient) -> None:
...
def test_delete_not_existing_network_service(api_client_read_write: TestClient) -> None:
