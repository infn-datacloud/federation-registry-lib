...
# TODO evaluate if it is possible to not use skip
@fixture
...
def db_flavor_simple(
    ...
    if len(projects) == 1:
        pytest.skip(
            "Case with only one project in the provider is a sub-case of the \
                    case with a provider with multiple projects."
        )
